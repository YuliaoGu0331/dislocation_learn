# 6. 其他晶体结构中的位错——详细版（Dislocations in Other Crystal Structures）
...
> [!info] 版本说明
> 本文为详细版，以最初原始笔记及其 30 张截图为内容基线，保留原文中的特例、教材旁注和反应案例，并统一修正术语、公式与晶体学记号。需要快速复习时可阅读 [6_others_simplified.md](6_others_simplified.md)。
...
1. 基面三角形 $ABC$ 的六个有向边 $AB$、$BC$、$CA$、$BA$、$CB$、$AC$ 表示六个 $a$ 型完整位错。
2. $ST$ 与 $TS$ 表示垂直基面的两个 $c$ 型完整位错。
3. $SA$、$SB$、$SC$、$TA$、$TB$、$TC$ 及其反向表示十二个 $a+c$ 型完整位错。例如 $SA$ 可理解为 $ST+TA$；几何上也等于相应两条线段中点连线的两倍。
4. $A\sigma$、$B\sigma$、$C\sigma$ 及反向矢量表示基面 Shockley 型不全位错。
5. $\sigma S$、$\sigma T$、$S\sigma$、$T\sigma$ 表示伯格斯矢量垂直基面的 $c/2$ 型不全位错。
6. $AS$、$BS$ 等可视为上述两类不全位错的组合。即使某些矢量连接两个原子位置，它们仍可能是不全位错，因为端点的局部化学/几何环境并不完全等价，该矢量不是完整晶格平移。
...
教材指出，许多以基面滑移为主的 HCP 金属，其基面层错能估计值高于典型 FCC 金属；但该趋势不是单纯由几何密排决定，更精确的 $\gamma$ 值需要第一性原理或实验测量。

#### $\gamma$-surface 的对称性

FCC 的 $\{111\}$ 面 $\gamma$-surface 沿三个等价 $\frac16\langle11\bar{2}\rangle$ 方向具有镜面对称性，因此这些方向上容易出现对称的极值点。HCP 基面 $I_2$ 层错的 $\gamma$-surface 具有类似的三重对称约束，这解释了为什么若一个 $\frac13\langle10\bar{1}0\rangle$ 层错位移稳定，其他两个对称等价方向也会出现相应极值。

#### 柱面层错候选：硬球模型特例

对一阶柱面 $\{10\bar{1}0\}$，几何硬球模型曾提出可能存在位移矢量为

$$
\frac1{18}\langle24\bar{6}3\rangle
\qquad\text{或}\qquad
\frac19\langle11\bar{2}0\rangle
$$

的稳定层错。前者可在理想硬球晶格中形成几何稳定错排；后者会使层错带相邻柱面局部接近 BCC 的 $\{112\}$ 堆垛。它们是用于理解柱面核心的模型候选，不应当作所有 HCP 金属中都稳定存在的通用层错。
...
更具体地说，刃位错只要所在面存在稳定层错极小值，就可分解并形成扩展核心；螺位错则可同时比较多个包含 $\mathbf b_a$ 的候选面。如果稳定核心主要扩展在基面，而宏观滑移要求它沿柱面运动，位错必须先形成局部收缩段再在柱面传播；若稳定核心主要扩展在柱面，则基面滑移会遇到类似的核心重构障碍。因此“某金属偏好基面还是柱面滑移”常由螺位错核心控制，而不只是由刃位错几何决定。
...
硬球模型还提出，位于二阶锥面 $\{11\bar{2}2\}$ 上的 $a+c$ 位错可近似分解为四个伯格斯矢量大小相近的不全位错。其核心可展宽到连续三个 $\{11\bar{2}2\}$ 面，并形成三片相邻错排。最前方的不全位错可表现为**带状孪晶位错（zonal twinning dislocation）**：它相当于相干孪晶界上的多层台阶，运动时直接生成一段孪晶。这个模型提示 $\langle11\bar{2}3\rangle\{11\bar{2}2\}$ 滑移与孪晶可能具有共同核心机制，但真实核心未必严格符合硬球四分解图像。
...
除空位片外，沉淀物或间隙原子片也可等效为插入额外原子层，先生成 $E$ 型层错，再通过位错反应转化为 $I_1$ 层错。Frank 型不可滑移环是否被剪切消除，与 FCC 中的情形类似，取决于环尺寸、层错能、成核势垒和外应力。

> [!note] 基面与柱面原子密度的几何特例
> 在理想几何比较中，只有当 $c/a>\sqrt3$ 时，基面面密度才高于所比较的一阶柱面面密度。该结果说明“基面必然最密排”并非对任意轴比都成立，但实际滑移偏好仍需结合键合与核心能，不能只用面密度判断。
...
BCC 中缺少由稳定层错带分隔的扩展螺位错，因此交滑移不需要像 FCC 那样先消除一条稳定层错带；从几何约束看更容易切换滑移面。但螺位错非平面核心的重构势垒仍可能很高，所以“没有层错”不等于螺位错本身易动。
...
相较之下，刃型和一般混合型线段的核心更接近平面化，对不产生滑移 Peach–Koehler 力的应力分量通常不如螺位错敏感，且所需滑移应力显著更低。这一迁移率差异解释了为何加载后非螺型线段迅速离开，而长螺型线段在显微组织中累积。

### 6.3.4 $\langle100\rangle$ 位错与变形孪晶

除 $\frac a2\langle111\rangle$ 位错外，BCC 中有时也能观察到 $a\langle100\rangle$ 位错；在立方晶体中原文所说的 $\langle001\rangle$ 与 $\langle100\rangle$ 属于同一方向族。它可由两条 $\frac a2\langle111\rangle$ 位错反应形成，例如：
...
单原子高度刃型 jog 的 $\pm e/2$ 电荷不能由只携带整数倍基本电荷的单个点缺陷完全中和，因此相邻带电 jog 会发生明显库仑相互作用；两个同号高度单元组合成偶数高度 jog 后才可能整体中性。螺位错运动使电荷沿位错线方向重新排列，所以螺位错的 kink 与 jog 都可能带 $\pm e/4$。空位在 jog 附近易于生成或被吸收，过量空位及其他补偿缺陷会屏蔽位错的净有效电荷。
...
这与 FCC 扩展位错“两个 Shockley 不全位错夹一条层错带”的几何形式相似，但中间缺陷的物理来源不同：FCC 层错破坏几何堆垛次序，APB 则破坏有序合金中不同元素所占子晶格的相位。超不全位错的伯格斯矢量继承自无序 FCC 母相，而两个超不全位错的合矢量才是有序超点阵的完整平移。

![$L1_2$ 超点阵在 $\{111\}$ 与 $\{100\}$ 面上的原子排列和 APB 位移](assets/6_others/l12-apb-slip-planes.png)

在 $\{111\}$ 面上，从一个有序位置 $X$ 平移 $\frac a2\langle110\rangle$ 到 $Y$ 会交换局部子晶格并形成 APB，再由 $Y$ 平移到下一个等价有序位置 $Z$ 才恢复长程有序。在 $\{100\}$ 面上，相同第一步平移可保持 APB 两侧最近邻键的主要组成，APB 能更多来自次近邻变化，因此许多 $L1_2$ 合金满足

$$
\gamma_{\mathrm{APB}}^{\{100\}}
<\gamma_{\mathrm{APB}}^{\{111\}}.
$$
...
因此典型温度响应可分为三个阶段：低温下行为近似普通 FCC 合金，CRSS 对温度依赖较弱；中温区 Kear–Wilsdorf 锁形成概率上升，CRSS 随温度升高而增加；更高温度下热激活解锁、攀移和扩散软化逐渐占优，强度重新下降。具体峰值温度和机理取决于 APB 能、合金成分与加载方向。
...
并在 $\{111\}$ 面上运动。完整位错线常沿 $\langle110\rangle$ 低能方向排列，与伯格斯矢量成 $0^\circ$（螺型）或 $60^\circ$；分解后的 Shockley 不全位错还常讨论 $30^\circ$ 和 $90^\circ$ 等线取向。
...
原始笔记所述的一种转换图像是：shuffle-set 位错因缺少低能层错，局部线段可在相邻 glide-set 面产生不全位错，同时携带或留下空位/间隙原子，从而在 glide set 上形成层错带。该过程需要点缺陷参与，因而比普通保守滑移复杂；反过来，攀移和点缺陷吸收也可把 shuffle 与 glide 核心相互转化。这里应把它理解为可能的核心/点缺陷耦合路径，而不是所有 shuffle 位错都必然遵循的唯一机制。
